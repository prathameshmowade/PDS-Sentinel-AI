...
import OfficialGrievanceManagement from './components/Dashboard/OfficialGrievanceManagement';
  ...
  return (
    <div className="min-h-screen relative bg-slate-50 dark:bg-[#0b0f17] text-slate-900 dark:text-slate-100 flex flex-col font-sans selection:bg-sky-500 selection:text-white transition-colors duration-300">
      ...
      {!currentUser ? (
        ...
        /* 2. CITIZEN DEDICATED VIEW (ISOLATED ROLE) */
        <>
          <CitizenHeader
            ...
            currentLang={citizenLang}
            onChangeLang={setCitizenLang}
          />

          <main className="relative z-10 flex-1 max-w-5xl w-full mx-auto px-4 sm:px-6 py-6">
            <CitizenPortal 
              fpsList={fpsList} 
              currentLang={citizenLang}
              onLangChange={setCitizenLang}
            />
          </main>

          <footer className="relative z-10 border-t border-slate-200 dark:border-slate-800/80 bg-white/90 dark:bg-[#080b11]/90 backdrop-blur-xs py-4 text-[11px] text-slate-500 transition-colors text-center">
            <div className="max-w-5xl mx-auto px-4 flex flex-col sm:flex-row items-center justify-between gap-2">
              <div>
                <span className="font-semibold text-slate-700 dark:text-slate-400">MahaPDS Beneficiary Portal</span> • National Food Security Act (NFSA 2013)
              </div>
              <div className="flex items-center space-x-3 font-mono text-[10px]">
                <span>Toll Free: 1967</span>
                <span>•</span>
                <span>ONORC Portability Active</span>
              </div>
            </div>
          </footer>
        </>
      ) : (
        /* 3. GOVERNMENT OFFICIAL / ENFORCEMENT VIEW */
        <>
          {/* Global Official Header */}
          <Header
            activeTab={activeTab}
          ...
          <main className="relative z-10 flex-1 max-w-7xl w-full mx-auto px-4 sm:px-6 lg:px-8 py-5">
            ...
            {/* TAB 4: OFFICIAL CITIZEN GRIEVANCE & COMPLAINT MANAGEMENT */}
            {activeTab === 'grievances' && (
              <OfficialGrievanceManagement onRefreshData={loadAllData} />
            )}
